...
請改用具備畫面感的「意象與情境模塊」來建構 sentenceSkeleton（句型骨架）。

- ❌ 錯誤示範（死板填空）：在 [形容詞] 的 [自然現象] 中，[主詞] [動作]。
- ✅ 正確示範（意象模塊）：在【設定一個有光線或微風的自然場景】，【主角用擬人的姿態悄悄登場】，【它在心裡許下了一個什麼樣的純真願望】。

骨架與範文的提示語必須嚴格遵守以下 4 個條件：
1. 有畫面感：引導學生思考場景與動作。
2. 有情感動機：引導學生寫出主角的「渴望」或「情緒」。
3. 保留節奏：括號內的引導語要精煉，讀起來依然要有詩的節奏感。
4. ⚠️【物性邏輯防呆 (CRITICAL)】：仿寫的意象與動作，必須「絕對符合」主角真實的物理特性！嚴禁盲目抄襲原詩的專屬意象。
   - ❌ 災難示範：鉛筆裝滿了「墨水」、鉛筆像小樹苗一樣「發芽」。（鉛筆是筆芯，沒有墨水也不會發芽！）
   - ✅ 正確示範：鉛筆「削尖了頭」、在白紙上「摩擦出黑色的軌跡」、橡皮擦「犧牲自己留下雪白的碎屑」。`
};
