...
3. **場景化示範**：示範例句必須具備該主題的「具象感」，讓學生一眼就能看出不同路徑的寫作特色。`,
  ...
  poemRules: `
## 🚨 詩歌仿寫最高鐵律：字句精煉與結構鏡像 (CRITICAL POETRY RULES)
因為這是「詩歌」，絕對禁止寫成冗長、囉嗦的「記敘文」或「散文」！

1. **行數鏡像**：仿寫範文的行數必須與「原詩」的該段落行數相同（最多允許增減一行）。
2. **字數限制與概念完整**：每一行絕對不可超過 15 個字！但必須完整表達該段落的「核心概念」與「寫作重點」，不可因字數限制而讓語意破碎。
3. **拒絕繁雜堆砌**：嚴禁使用過多形容詞或冗長的動作描述，請保持童詩的輕快節奏。
   - ✅ 正確（精煉）：在安靜的筆盒裡， / 小鉛筆削尖了頭， / 輕輕踮起腳尖， / 想要畫出美麗的彩虹。
   - ❌ 錯誤（囉嗦）：在亮亮的鉛筆盒裡面，小鉛筆安靜地躺著，牠的筆頭輕輕晃動，好像悄悄許下願望... (這是散文，不是詩！)
4. **意象模塊法**：請用具備畫面感的「意象與情境模塊」來建構骨架。
5. ⚠️【物性邏輯防呆】：仿寫的意象必須「絕對符合」主角真實的物理特性。`
};
...
2. **fullExample (完整範文)**：【🚨 詩歌仿寫格式鐵律】這是一首給小學生的「短詩」！
   - **強制行數**：行數必須與原詩段落相同（最多上下浮動一行）。
   - **強制字數與內容**：每一行不可超過 15 個字，但必須將大綱設定的「核心概念」完整表達。
   - **風格限制**：必須像原詩一樣精煉、輕快，【絕對禁止】寫成囉嗦的長篇大論。
   - **格式限制**：每一行結尾必須加上換行符號 \\n。🚨 【絕對禁止】出現【】括號。
...
${genre.includes('詩') ? `3. **詩歌防呆**：分歧路徑的範文 (fullExample) 行數必須與原詩相同 (最多增減一行)，每行不超過 15 字，且須完整表達核心概念，絕對禁止寫成散文！` : ''}
